...
export default function EditLinks() {
  ...
  useEffect(() => {
    ...
    api.get('/pages/links', { signal: controller.signal })
      .then(r => {
        setPage(r.data.page || { title: 'Links', content: '' })
        setError(null)
      })
      .catch(err => {
    ...
    <div className="mx-auto max-w-4xl p-6">
      ...
      <div className="mb-4">
        ...
        <textarea
          ...
          rows={4}
          className="w-full rounded-lg bg-gray-50 border border-brandSteel/50 px-3 py-2 text-gray-900 resize-y focus:ring-2 focus:ring-brandGreen/50 focus:border-brandGreen shadow-sm"
          placeholder="Links"
        />
      </div>
