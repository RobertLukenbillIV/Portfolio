...
export default function PostEditor({ mode }: { mode: 'create' | 'edit' }) {
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      ...
      <input value={title} onChange={e=>setTitle(e.target.value)} className="w-full mb-3 rounded-lg bg-brandFoam/40 border border-brandSteel/30 px-3 py-2 text-mocha"/>

      <label className="block text-brandSteel mb-1">Excerpt</label>
      <textarea value={excerpt} onChange={e=>setExcerpt(e.target.value)} rows={3} className="w-full mb-3 rounded-lg bg-brandFoam/40 border border-brandSteel/30 px-3 py-2 text-mocha"/>

      <label className="block text-brandSteel mb-1">Cover Image URL</label>
      <input value={coverUrl} onChange={e=>setCoverUrl(e.target.value)} className="w-full mb-4 rounded-lg bg-brandFoam/40 border border-brandSteel/30 px-3 py-2 text-mocha"/>
      ...
      <div className="mt-4 flex gap-2">
        ...
        <button onClick={()=>navigate(-1)} className="px-4 py-2 rounded-lg bg-brandMint/20 text-mocha hover:opacity-90">Cancel</button>
      </div>
    </div>
  )
}
